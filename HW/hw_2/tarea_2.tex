...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
    \hlstd{digits} \hlkwb{=} \hlstd{df}\hlopt{$}\hlstd{val}
    \hlkwd{runs.test}\hlstd{(digits)}
\end{alltt}
\begin{verbatim}
## 
## 	Runs Test
## 
## data:  digits
## statistic = 1.3559, runs = 444850, n1 = 499990, n2 = 399640, n =
## 899630, p-value = 0.1751
## alternative hypothesis: nonrandomness
\end{verbatim}
\begin{alltt}
    \hlcom{#Test de Bartlett para ruido blanco}
    \hlkwd{bartlettB.test}\hlstd{(digits)}
\end{alltt}
\begin{verbatim}
## 
## 	Bartlett B Test for white noise
## 
## data:  
## = 1.7677, p-value = 0.003863
\end{verbatim}
\begin{alltt}
    \hlcom{#Test de Box-Pierce}
    \hlkwd{Box.test}\hlstd{(digits,}\hlkwc{lag}\hlstd{=}\hlnum{5}\hlstd{,}\hlkwc{type}\hlstd{=}\hlstr{"Box-Pierce"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
## 	Box-Pierce test
## 
## data:  digits
## X-squared = 11.033, df = 5, p-value = 0.05073
\end{verbatim}
\begin{alltt}
    \hlcom{#Test de Ljung-Box}
    \hlkwd{Box.test}\hlstd{(digits,}\hlkwc{lag}\hlstd{=}\hlnum{5}\hlstd{,}\hlkwc{type}\hlstd{=}\hlstr{"Ljung-Box"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
## 	Box-Ljung test
...
\begin{alltt}
    \hlcom{#Obteniendo la función de autocorrelación}
    \hlstd{ACF} \hlkwb{=} \hlkwd{Acf}\hlstd{(digits)}
\end{alltt}
\end{kframe}
...
\begin{kframe}\begin{alltt}
    \hlcom{#Realizando el test de Bartlett}
    \hlstd{rho} \hlkwb{<-} \hlstd{ACF}\hlopt{$}\hlstd{acf[}\hlnum{2}\hlstd{]}
    \hlstd{bt} \hlkwb{<-} \hlkwd{sqrt}\hlstd{(}\hlkwd{length}\hlstd{(digits))}\hlopt{*}\hlstd{rho}
    \hlstd{p.value} \hlkwb{=} \hlnum{1}\hlopt{-}\hlkwd{pnorm}\hlstd{(bt)}\hlopt{/}\hlnum{2}
    \hlkwd{print}\hlstd{(p.value)}
\end{alltt}
\begin{verbatim}
